#include "ftest_imp_claim_oracle.h"
...
#define IMP_CLAIM_ORACLE_VERSION 1
...
#define IMP_CLAIM_ORACLE_LEVEL 9005
...
#define CLAIM_MAX_TICKS 600

static FILE* imp_claim_jsonl = NULL;
static struct Thing* imp_claim_imp = NULL;
static int imp_claim_countdown = -1; // -1 until the tile is claimed; then counts POST_CLAIM_TICKS down to 0.

// forward declaration
FTestActionResult ftest_imp_claim_oracle_action__run(struct FTestActionArgs* const args);
...
TbBool ftest_imp_claim_oracle_init()
{
    ...
    ftest_append_action(ftest_imp_claim_oracle_action__run, SPAWN_DELAY_TURNS, NULL);
    return true;
}
...
static const char* imp_claim_out_dir(void)
{
...
static void imp_claim_dump(FILE* f, struct Thing* imp)
{
...
FTestActionResult ftest_imp_claim_oracle_action__run(struct FTestActionArgs* const args)
{
    if (args->times_executed == 0)
    {
        ...
        if (level != IMP_CLAIM_ORACLE_LEVEL)
        {
            FTEST_FAIL_TEST("imp claim oracle: expected level %d (M-claim), got %ld — no claim tile",
                IMP_CLAIM_ORACLE_LEVEL, level);
        ...
        imp_claim_imp = ftest_util_create_creature(sx, sy, PLAYER0, 1, get_players_special_digger_model(PLAYER0));
        if (thing_is_invalid(imp_claim_imp))
        {
            FTEST_FAIL_TEST("imp claim oracle: could not spawn the imp at subtile (%d,%d)", SPAWN_STL_X, SPAWN_STL_Y);
            return FTRs_Go_To_Next_Action;
        }
        // Settle the floor under the spawn tile before the game loop takes over (mirrors the movement oracle).
        move_thing_in_map(imp_claim_imp, &imp_claim_imp->mappos);
        ...
        if (target->kind != SlbT_PATH)
        {
            FTEST_FAIL_TEST("imp claim oracle: target slab (%d,%d) is kind %d, expected SlbT_PATH",
                TILE_SLB_X, TILE_SLB_Y, (int)target->kind);
            delete_thing_structure(imp_claim_imp, 0);
            imp_claim_imp = NULL;
            return FTRs_Go_To_Next_Action;
        }

        char path[512];
        snprintf(path, sizeof(path), "%s/oracle_imp_claim.jsonl", imp_claim_out_dir());
        imp_claim_jsonl = fopen(path, "w");
        if (imp_claim_jsonl == NULL)
        {
            FTEST_FRAMEWORK_ABORT("imp claim oracle: failed to open '%s'", path);
            delete_thing_structure(imp_claim_imp, 0);
            imp_claim_imp = NULL;
            return FTRs_Go_To_Next_Action;
        }
    ...
    struct SlabMap* slb = get_slabmap_block(TILE_SLB_X, TILE_SLB_Y);
    if (slb->kind == SlbT_CLAIMED && imp_claim_countdown < 0)
        imp_claim_countdown = POST_CLAIM_TICKS;

    if (imp_claim_countdown == 0 || (long)args->times_executed >= CLAIM_MAX_TICKS)
    {
        FTESTLOG("imp claim oracle: dump complete (%ld records, final block kind %d)",
            (long)args->times_executed + 1, (int)slb->kind);
        fclose(imp_claim_jsonl);
        imp_claim_jsonl = NULL;
        delete_thing_structure(imp_claim_imp, 0);
        imp_claim_imp = NULL;
        return FTRs_Go_To_Next_Action;
    }
    if (imp_claim_countdown > 0)
        imp_claim_countdown--;
